...
/// Persisted viewport state for one scenario canvas.
///
/// `pan` is expressed in screen-space points relative to the canvas viewport
/// origin. `zoom` is a scale factor where `1.0` means 100%. Consumers are
/// responsible for applying their own supported zoom limits when rendering.
...
impl CanvasView {
    /// Whether this view is the implicit, backward-compatible viewport.
...
pub struct ComposerCanvas {
    ...
    /// Legacy v1 canvas wiring. Workflow graph v3 uses `WorkflowGraph::edges`
    /// as its sole execution topology, so this map is accepted on import but
    /// is never emitted by current writers.
...
mod tests {
    use super::*;
    use crate::automation::{default_step, ActionKind, ActionNode, GraphNode, WorkflowGraph};
    use crate::rules::Platform;

    fn task(id: &str) -> Task {
        let step = default_step(ActionKind::InspectPath, format!("{id}-action")).unwrap();
    ...
    fn project_round_trips_nested_entries_and_canvas_layout() {
        ...
        project.canvases.insert(
            ...
            ComposerCanvas {
                ...
                view: CanvasView {
                    pan: CanvasPoint { x: -125.5, y: 48.0 },
                    zoom: 1.25,
                },
            },
        );
        ...
        assert!(reparsed.canvases["nested-scenario"].parents.is_empty());
        assert_eq!(reparsed.canvases["nested-scenario"].view.zoom, 1.25);
        assert_eq!(reparsed.canvases["nested-scenario"].view.pan.x, -125.5);
        assert!(!yaml.contains("parents:"));
    }

    #[test]
    fn canvas_view_defaults_for_legacy_yaml_and_legacy_parents_are_import_only() {
        let legacy: ComposerCanvas = serde_yaml::from_str(
            r#"
positions:
  start: { x: 80.0, y: 250.0 }
  inspect: { x: 366.0, y: 250.0 }
parents:
  inspect: start
"#,
        )
        ...
        assert_eq!(legacy.parents["inspect"], "start");

        let normalized = serde_yaml::to_string(&legacy).unwrap();
        assert!(!normalized.contains("view:"));
        assert!(!normalized.contains("parents:"));
    }
